...
func StartSequences(guiButtons chan common.ALight) {
	...
	go sequence.PlaySequence(*sequences[0], 0, pad, eventsForLauchpad, guiButtons, pattens, dmxController, fixturesConfig, sequenceChannels, soundTriggers)
	go sequence.PlaySequence(*sequences[1], 1, pad, eventsForLauchpad, guiButtons, pattens, dmxController, fixturesConfig, sequenceChannels, soundTriggers)
	go sequence.PlaySequence(*sequences[2], 2, pad, eventsForLauchpad, guiButtons, pattens, dmxController, fixturesConfig, sequenceChannels, soundTriggers)
	go sequence.PlaySequence(*sequences[3], 3, pad, eventsForLauchpad, guiButtons, pattens, dmxController, fixturesConfig, sequenceChannels, soundTriggers)

	// Light up any existing presets.
	presets.InitPresets(eventsForLauchpad, guiButtons, presetsStore)
	...
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 0, Y: -1, Brightness: 255, Red: 200, Green: 0, Blue: 255})

	// Light the static color buttons.
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 1, Y: -1, Brightness: 255, Red: 255, Green: 0, Blue: 0})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 2, Y: -1, Brightness: 255, Red: 0, Green: 255, Blue: 0})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 3, Y: -1, Brightness: 255, Red: 0, Green: 0, Blue: 255})

	// Light top functions.
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 4, Y: -1, Brightness: 255, Red: 3, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 5, Y: -1, Brightness: 255, Red: 3, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 6, Y: -1, Brightness: 255, Red: 3, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 7, Y: -1, Brightness: 255, Red: 3, Green: 255, Blue: 255})

	// Light the save, start, stop and blackout buttons.
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 8, Y: 5, Brightness: full, Red: 255, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 8, Y: 6, Brightness: full, Red: 255, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 8, Y: 7, Brightness: full, Red: 255, Green: 255, Blue: 255})
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 8, Y: 8, Brightness: full, Red: 255, Green: 255, Blue: 255})

	// Initialise the flood button to be green.
	common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 7, Y: 3, Brightness: full, Red: 255, Green: 0, Blue: 0})

	// Light the first sequence as the default selected.
	selectedSequence := 0
	common.SequenceSelect(eventsForLauchpad, guiButtons, selectedSequence)
	...
	for {
		...
		if hit.X == 0 && hit.Y == -1 && sequences[selectedSequence].Type != "scanner" {
			...
			if sequences[selectedSequence].Functions[common.Function5_Color].State &&
				...
				ShowRGBColorSelectionButtons(selectedSequence, *sequences[selectedSequence], eventsForLauchpad, guiButtons)

				continue
			}

			launchpad.ClearAll(pad, presetsStore, eventsForLauchpad, guiButtons, commandChannels)
			allFixturesOff(eventsForLauchpad, dmxController, fixturesConfig)
			presets.ClearPresets(eventsForLauchpad, guiButtons, presetsStore)
			presets.InitPresets(eventsForLauchpad, guiButtons, presetsStore)
		...
		if hit.X == 8 && hit.Y == 3 {
			...
			if flood { // If we are flood already then tell the sequence to stop flood.
				...
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 255, Green: 255, Blue: 255})
				...
				for selectedSequence := range sequences {
					...
					common.HideFunctionButtons(selectedSequence, eventsForLauchpad, guiButtons)
		...
		if hit.X == 8 && hit.Y == 4 {
			...
			if savePreset {
				savePreset = false
				presets.InitPresets(eventsForLauchpad, guiButtons, presetsStore)
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: 8, Y: 4, Brightness: full, Red: 255, Green: 255, Blue: 255})
				continue
			}
			presets.InitPresets(eventsForLauchpad, guiButtons, presetsStore)
		...
		if hit.X < 8 && (hit.Y > 3 && hit.Y < 7) {
			...
			if savePreset {
				...
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 255, Green: 0, Blue: 0})
				...
				if presetsStore[fmt.Sprint(hit.X)+","+fmt.Sprint(hit.Y)] {
					...
					common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 3, Green: 0, Blue: 0})
					presets.InitPresets(eventsForLauchpad, guiButtons, presetsStore)
					launchpad.FlashLight(hit.X, hit.Y, 0x0d, 0x78, eventsForLauchpad)
		...
		if hit.X == 8 && hit.Y == 0 {
			...
			selectedSequence = 0
			HandleSelect(sequences, selectedSequence, &selectedFixture, &followingAction, &editFixtureSelectionMode, eventsForLauchpad, selectButtonPressed, &editFixtureSelectionMode, functionButtons,
				functionSelectMode, editSequenceColorsMode, editScannerColorsMode, editGoboSelectionMode, editStaticColorsMode, editPattenMode, commandChannels, sequenceChannels, fixturesConfig, guiButtons)
		...
		if hit.X == 8 && hit.Y == 1 {
			...
			selectedSequence = 1
			HandleSelect(sequences, selectedSequence, &selectedFixture, &followingAction, &editFixtureSelectionMode, eventsForLauchpad, selectButtonPressed, &editFixtureSelectionMode, functionButtons,
				functionSelectMode, editSequenceColorsMode, editScannerColorsMode, editStaticColorsMode, editGoboSelectionMode, editPattenMode, commandChannels, sequenceChannels, fixturesConfig, guiButtons)
		...
		if hit.X >= 0 && hit.X < 8 &&
			...
			common.ShowFunctionButtons(*sequences[selectedSequence], selectedSequence, eventsForLauchpad, guiButtons)
			...
			if editPattenMode[selectedSequence] {
				...
				ShowPattenSelectionButtons(selectedSequence, *sequences[selectedSequence], eventsForLauchpad, guiButtons)
			}
			...
			if editSequenceColorsMode[selectedSequence] && sequences[selectedSequence].Type == "rgb" {
				time.Sleep(500 * time.Millisecond) // But give the launchpad time to light the function key purple.
				common.HideFunctionButtons(selectedSequence, eventsForLauchpad, guiButtons)
				ShowRGBColorSelectionButtons(selectedSequence, *sequences[selectedSequence], eventsForLauchpad, guiButtons)
			}
			...
			if editSequenceColorsMode[selectedSequence] && sequences[selectedSequence].Type == "scanner" {
				time.Sleep(500 * time.Millisecond) // But give the launchpad time to light the function key purple.
				common.HideFunctionButtons(selectedSequence, eventsForLauchpad, guiButtons)
				editFixtureSelectionMode = true
				followingAction = "ShowScannerColorSelectionButtons"
				selectedFixture = ShowSelectFixtureButtons(selectedSequence, selectedFixture, *sequences[selectedSequence], eventsForLauchpad, fixturesConfig, followingAction, guiButtons)
			}
			...
			if editGoboSelectionMode[selectedSequence] {
				...
				common.HideFunctionButtons(selectedSequence, eventsForLauchpad, guiButtons)
				editFixtureSelectionMode = true
				followingAction = "ShowGoboSelectionButtons"
				selectedFixture = ShowSelectFixtureButtons(selectedSequence, selectedFixture, *sequences[selectedSequence], eventsForLauchpad, fixturesConfig, followingAction, guiButtons)
			}
		...
		if hit.X >= 0 &&
			...
			common.LightOn(eventsForLauchpad, guiButtons, common.ALight{
				X:          hit.X,
				Y:          hit.Y,
		...
		if hit.X == 1 && hit.Y == -1 && sequences[selectedSequence].Type != "scanner" {
			...
			if sequences[selectedSequence].Functions[common.Function6_Static].State {
				...
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: staticButtons[selectedSequence].Color.R, Green: 0, Blue: 0})
		...
		if hit.X == 2 && hit.Y == -1 && sequences[selectedSequence].Type != "scanner" {
			...
			if sequences[selectedSequence].Functions[common.Function6_Static].State {
				...
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 0, Green: staticButtons[selectedSequence].Color.G, Blue: 0})
				updateStaticLamp(selectedSequence, staticButtons[selectedSequence], commandChannels)
				continue
			}
		}
		...
		if hit.X == 3 && hit.Y == -1 && sequences[selectedSequence].Type != "scanner" {
			...
			if sequences[selectedSequence].Functions[common.Function6_Static].State {
				...
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 0, Green: 0, Blue: staticButtons[selectedSequence].Color.B})
				updateStaticLamp(selectedSequence, staticButtons[selectedSequence], commandChannels)
				continue
			}
		}
		...
		if hit.X >= 0 && hit.X < 8 && hit.Y != -1 &&
			...
			cmd := common.Command{
				...
				Args: []common.Arg{
					...
					{Name: "SelectedFixture", Value: selectedFixture},
				},
			}
			common.SendCommandToSequence(selectedSequence, cmd, commandChannels)
			...
			fixture.MapFixturesColorOnly(sequences[selectedSequence], dmxController, fixturesConfig, scannerColor)

			// We call ShowScannerColorSelectionButtons here so the selections will flash as you press them.
			ShowScannerColorSelectionButtons(selectedSequence, selectedFixture, *sequences[selectedSequence], eventsForLauchpad, fixturesConfig, guiButtons)
		...
		if hit.X >= 0 && hit.X < 8 && hit.Y != -1 &&
			...
			if followingAction == "ShowGoboSelectionButtons" {
				ShowGoboSelectionButtons(selectedSequence, *sequences[selectedSequence], eventsForLauchpad, guiButtons)
			}
			if followingAction == "ShowScannerColorSelectionButtons" {
				err := ShowScannerColorSelectionButtons(selectedSequence, selectedFixture, *sequences[selectedSequence], eventsForLauchpad, fixturesConfig, guiButtons)
				if err != nil {
					fmt.Printf("error: %v\n", err)
					common.RevealSequence(selectedSequence, commandChannels)
				}
		...
		if hit.X >= 0 && hit.X < 8 && hit.Y != -1 &&
			...
			fixture.MapFixturesColorOnly(sequences[selectedSequence], dmxController, fixturesConfig, selectedGobo)
		...
		if hit.X == 8 && hit.Y == 7 {
			...
			if !blackout {
				...
				common.SendCommandToAllSequence(selectedSequence, cmd, commandChannels)
				common.LightOn(eventsForLauchpad, guiButtons, common.ALight{X: hit.X, Y: hit.Y, Brightness: full, Red: 0, Green: 0, Blue: 0})
...
func HandleSelect(sequences []*common.Sequence,
	...
	editFixtureSelectionMode *bool,
	eventsForLauchpad chan common.ALight,
	selectButtonPressed []bool,
	selectFixtureActive *bool,
	...
	if functionSelectMode[selectedSequence] && !selectButtonPressed[selectedSequence] &&
		...
		if sequences[selectedSequence].Functions[common.Function5_Color].State && sequences[selectedSequence].Type == "rgb" {
			if debug {
	...
	if !selectButtonPressed[selectedSequence] &&
		...
		if sequences[selectedSequence].Functions[common.Function1_Patten].State {
			...
			ClearPattenSelectionButtons(selectedSequence, *sequences[selectedSequence], eventsForLauchpad, guiButtons)
		...
		if !functionSelectMode[selectedSequence] && sequences[selectedSequence].Functions[common.Function5_Color].State && editSequenceColorsMode[selectedSequence] {
			unSetEditSequenceColorsMode(selectedSequence, sequences, functionSelectMode, editSequenceColorsMode, selectButtonPressed, commandChannels, eventsForLauchpad, guiButtons)
		}
		return
	}
	...
	if !functionSelectMode[selectedSequence] &&
		...
		common.MakeFunctionButtons(*sequences[selectedSequence], selectedSequence, eventsForLauchpad, guiButtons, functionButtons, channels)
...
func unSetEditSequenceColorsMode(selectedSequence int,
	...
	HideColorSelectionButtons(selectedSequence, *sequences[selectedSequence], selectedSequence, eventsForLauchpad, guiButtons)
}
...
func ShowRGBColorSelectionButtons(mySequenceNumber int, sequence common.Sequence, eventsForLauchpad chan common.ALight, guiButtons chan common.ALight) {
	...
	for myFixtureNumber, lamp := range sequence.AvailableSequenceColors {
		...
		if lamp.Flash {
			...
			gui.LightLamp(myFixtureNumber, mySequenceNumber, lamp.Color.R, lamp.Color.G, lamp.Color.B, sequence.Master, guiButtons)
		}
	}
}

// For the given sequence show the available scanner selection colors on the relevant buttons.
func ShowSelectFixtureButtons(mySequenceNumber int, selectedFixture int, sequence common.Sequence, eventsForLauchpad chan common.ALight, fixtures *fixture.Fixtures, action string, guiButtons chan common.ALight) int {
	...
	for fixtureNumber, lamp := range sequence.AvailableFixtures {
		...
		if fixtureNumber == selectedFixture {
			lamp.Flash = true
			selectedFixture = fixtureNumber
		}
		if lamp.Flash {
			code := common.GetLaunchPadColorCodeByRGB(lamp.Color)
			...
			gui.LightLamp(fixtureNumber, mySequenceNumber, lamp.Color.R, lamp.Color.G, lamp.Color.B, sequence.Master, guiButtons)
...
func ShowGoboSelectionButtons(mySequenceNumber int, sequence common.Sequence, eventsForLauchpad chan common.ALight, guiButtons chan common.ALight) {
...
// For the given sequence show the available scanner selection colors on the relevant buttons.
func ShowScannerColorSelectionButtons(mySequenceNumber int, selectedFixture int, sequence common.Sequence, eventsForLauchpad chan common.ALight, fixtures *fixture.Fixtures, guiButtons chan common.ALight) error {

	if debug {
		fmt.Printf("Show Scanner Color Selection Buttons,  Sequence is %d  fixture is %d   color is %d \n", mySequenceNumber, selectedFixture, sequence.ScannerColor[selectedFixture])
	}
	...
	if sequence.AvailableScannerColors[selectedFixture+1] == nil {
		for _, fixture := range fixtures.Fixtures {
			if fixture.Group == mySequenceNumber+1 {
				...
				gui.LightLamp(fixture.Number-1, mySequenceNumber, 0, 0, 0, sequence.Master, guiButtons)
			}
		}
	...
	for fixtureNumber, lamp := range sequence.AvailableScannerColors[selectedFixture+1] {
		...
		if fixtureNumber == sequence.ScannerColor[selectedFixture] {
			lamp.Flash = true
		}
		if lamp.Flash {
			code := common.GetLaunchPadColorCodeByRGB(lamp.Color)
...
func ClearPattenSelectionButtons(mySequenceNumber int, sequence common.Sequence, eventsForLauchpad chan common.ALight, guiButtons chan common.ALight) {
	...
	for myFixtureNumber := 0; myFixtureNumber < 4; myFixtureNumber++ {
		...
		gui.LightLamp(myFixtureNumber, mySequenceNumber, 0, 0, 0, sequence.Master, guiButtons)
	}
}
...
func HideColorSelectionButtons(mySequenceNumber int, sequence common.Sequence, selectedSequence int, eventsForLauchpad chan common.ALight, guiButtons chan common.ALight) {
