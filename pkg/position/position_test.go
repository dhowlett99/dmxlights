...
func Test_hasAllEmptyFixures(t *testing.T) {

	emptyFixtures := map[int]common.Fixture{
		0: {
			Color: common.Color{},
		},
		1: {
	...
	notEmptyFixtures := map[int]common.Fixture{
		0: {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hasAllEmptyFixures(tt.args.fixtures); got != tt.want {
				t.Errorf("hasAllEmptyFixures() = %v, want %v", got, tt.want)
			}
		})
	}
}
