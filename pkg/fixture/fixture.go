...
func FixtureReceiver(
	...
	for {
		...
		if cmd.Clear || cmd.Blackout {
			...
			lastColor = turnOffAllFixtures(cmd, myFixtureNumber, cmd.SequenceNumber, fixtures, dmxController, eventsForLaunchpad, guiButtons, dmxInterfacePresent)
			continue
		}
		...
		if cmd.Type == "rgb" {
			...
			if cmd.StartFlood && cmd.Label != "chaser" {
				...
				common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, lamp, cmd.Master, eventsForLaunchpad, guiButtons)
				continue
			}
			...
			if cmd.StopFlood && cmd.Label != "chaser" {
				...
				common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, common.Black, 0, eventsForLaunchpad, guiButtons)
				continue
			}
			...
			if cmd.RGBStatic {
				...
				if cmd.RGBStaticColors[myFixtureNumber].Enabled {
					...
					lastColor = lightStaticFixture(sequence, myFixtureNumber, eventsForLaunchpad, guiButtons, fixtures, true, dmxController, dmxInterfacePresent)
					continue
				}
			}
			...
			if cmd.RGBFadeUpStatic {
				...
				if cmd.RGBStaticColors[myFixtureNumber].Enabled {
					...
					fadeUpStaticFixture(sequence, myFixtureNumber, stopFadeUp, stopFadeDown, lastColor, eventsForLaunchpad, guiButtons, fixtures, true, dmxController, dmxInterfacePresent, fixtureStepChannel)
					continue
				} else {
					// This fixture is disabled, shut it off.
					lastColor = turnOffSingleFixture(myFixtureNumber, cmd.SequenceNumber, common.Black, fixtures, dmxController, dmxInterfacePresent)
				}
			}

			if !cmd.RGBStatic && cmd.RGBPlayStaticOnce && cmd.Label != "chaser" {
				if debug {
					fmt.Printf("Fixture:%d =====>  RGB Static Fade Down from LastColor %+v\n", myFixtureNumber, lastColor)
				}
				...
				go func() {
					...
					if lastColor != common.EmptyColor {
						for _, fade := range fadeDownValues {
							...
							color := common.GetColorNameByRGB(lastColor)
							// Find a suitable gobo based on the requested static lamp color.
							scannerGobo := FindGobo(myFixtureNumber, cmd.SequenceNumber, color, fixtures)
							// Find a suitable color wheel settin based on the requested static lamp color.
							scannerColor := FindColor(myFixtureNumber, cmd.SequenceNumber, color, fixtures)
							...
							common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, lastColor, fade, eventsForLaunchpad, guiButtons)
							if cmd.Label == "chaser" {
							...
							MapFixtures(false, false, cmd.SequenceNumber, myFixtureNumber, lastColor, common.SCANNER_MID_POINT, common.SCANNER_MID_POINT, 0, 0, 0, scannerGobo, scannerColor, fixtures, cmd.Blackout, fade, master, 0, cmd.Strobe, cmd.StrobeSpeed, dmxController, dmxInterfacePresent)
				...
				common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, common.Black, cmd.Master, eventsForLaunchpad, guiButtons)
				continue
			}
			...
			if fixture.Enabled {
				...
				if cmd.Label == "chaser" {
					...
					if !cmd.Hide {
						common.LightLamp(common.Button{X: myFixtureNumber, Y: scannerFixturesSequenceNumber}, fixture.Color, fixture.Brightness, eventsForLaunchpad, guiButtons)
					}
					...
					if !cmd.Hide {
						common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, fixture.Color, cmd.Master, eventsForLaunchpad, guiButtons)
					}
		...
		if cmd.Type == "scanner" {
			...
			if cmd.StartFlood {
				...
				turnOnFixtures(cmd, myFixtureNumber, cmd.SequenceNumber, fixtures, dmxController, eventsForLaunchpad, guiButtons, dmxInterfacePresent)
				common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, common.White, cmd.Master, eventsForLaunchpad, guiButtons)
			...
			if cmd.StopFlood {
				...
				common.LightLamp(common.Button{X: myFixtureNumber, Y: cmd.SequenceNumber}, common.Black, common.MIN_DMX_BRIGHTNESS, eventsForLaunchpad, guiButtons)
				continue
			}
			...
			if fixture.Enabled {
				...
				turnOffSingleFixture(myFixtureNumber, cmd.SequenceNumber, lastColor, fixtures, dmxController, dmxInterfacePresent)
			}
		}
	}
}
...
func fadeUpStaticFixture(sequence common.Sequence, myFixtureNumber int, StopFadeUp chan bool, StopFadeDown chan bool, lastColor common.Color, eventsForLaunchpad chan common.ALight, guiButtons chan common.ALight, fixturesConfig *Fixtures, enabled bool, dmxController *ft232.DMXController, dmxInterfacePresent bool, fixtureStepChannel chan common.FixtureCommand) {
	...
	go func() {
		...
		if lastColor != common.EmptyColor {
			for _, fade := range fadeDownValues {
				...
				if sequence.Label == "chaser" {
					// If we are a RGB chaser used as a shutter chasser apply fade values to the scanner's master dimmer channel because
					// scanners doesn't have a rgb color mixing capability so the wheel has to be faded using the master.
					master = int(float64(sequence.Master) / 100 * (float64(fade) / 2.55))
				}
				MapFixtures(false, false, sequence.Number, myFixtureNumber, lastColor, common.SCANNER_MID_POINT, common.SCANNER_MID_POINT, 0, 0, 0, scannerGobo, scannerColor, fixturesConfig, sequence.Blackout, fade, master, 0, sequence.Strobe, sequence.StrobeSpeed, dmxController, dmxInterfacePresent)
		...
		for _, fade := range fadeUpValues {
			...
			select {
			case <-StopFadeUp:
				turnOffSingleFixture(myFixtureNumber, sequence.Number, lastColor, fixturesConfig, dmxController, dmxInterfacePresent)
				return
			case <-time.After(10 * time.Millisecond):
			}
			if sequence.Label == "chaser" {
			...
			lastColor = MapFixtures(false, false, sequence.Number, myFixtureNumber, lamp.Color, common.SCANNER_MID_POINT, common.SCANNER_MID_POINT, 0, 0, 0, scannerGobo, scannerColor, fixturesConfig, sequence.Blackout, fade, master, 0, sequence.Strobe, sequence.StrobeSpeed, dmxController, dmxInterfacePresent)
...
// lightStaticFixture plays out a static color and returns the color.
func lightStaticFixture(sequence common.Sequence, myFixtureNumber int, eventsForLaunchpad chan common.ALight, guiButtons chan common.ALight, fixturesConfig *Fixtures, enabled bool, dmxController *ft232.DMXController, dmxInterfacePresent bool) (lastColor common.Color) {

	if debug {
		fmt.Printf("%d: lightStaticFixture fixture %d\n", sequence.Number, myFixtureNumber)
	}
	...
	if !sequence.Hide {
		if lamp.Flash {
			...
			common.FlashLight(common.Button{X: myFixtureNumber, Y: sequence.Number}, onColor, common.Black, eventsForLaunchpad, guiButtons)
		} else {
			common.LightLamp(common.Button{X: myFixtureNumber, Y: sequence.Number}, lamp.Color, sequence.Master, eventsForLaunchpad, guiButtons)
		}
	}
	...
	// Only play once, we don't want to flood the DMX universe with continual commands.
	sequence.PlayStaticOnce = false
...
func turnOffAllFixtures(cmd common.FixtureCommand, myFixtureNumber int, mySequenceNumber int, fixtures *Fixtures, dmxController *ft232.DMXController, eventsForLaunchpad chan common.ALight, guiButtons chan common.ALight, dmxInterfacePresent bool) (lastColor common.Color) {

	if debug {
		fmt.Printf("turnOffAllFixtures\n")
	}
...
func turnOnFixtures(cmd common.FixtureCommand, myFixtureNumber int, mySequenceNumber int, fixtures *Fixtures, dmxController *ft232.DMXController, eventsForLaunchpad chan common.ALight, guiButtons chan common.ALight, dmxInterfacePresent bool) common.Color {

	if debug {
		fmt.Printf("turnOnFixtures\n")
	}

	if !cmd.Hide {
		common.LightLamp(common.Button{X: myFixtureNumber, Y: mySequenceNumber}, common.White, common.MAX_DMX_BRIGHTNESS, eventsForLaunchpad, guiButtons)
	}
	pan := 128
	tilt := 128
...
func turnOffSingleFixture(myFixtureNumber int, mySequenceNumber int, lastColor common.Color, fixtures *Fixtures, dmxController *ft232.DMXController, dmxInterfacePresent bool) common.Color {

	if debug {
		fmt.Printf("turnOffSingleFixture: Sequence No %d Fixture No %d\n", mySequenceNumber, myFixtureNumber)
	}
